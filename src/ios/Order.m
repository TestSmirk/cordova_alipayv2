...
- (NSString *)description {
    ...
    // NOTE: 增加不变部分数据@"seller_id":_seller_id?:@"",
...
//biz_content":_biz_content.description?:@"",意思是_biz_content.description不存在时，biz_content为空
...
- (NSString*)encodeValue:(NSString*)value
{
    ...
    if (value.length > 0) {
        encodedValue = (__bridge_transfer  NSString*)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault,
         (__bridge CFStringRef)value, NULL, (__bridge CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8 );
    }
    return encodedValue;
}
@end
